...
def test_or_run(flag, cmd, user_name, project_type, sql_type='insert', logfile = '/share/nas1/liuw/sql.log'):
    if sql_type == 'insert':
        temp_file = '/home/cloudpub/liuw_sql/sql_insert_'+user_name+'_'+project_type+'.txt'
    else:
        temp_file = '/home/cloudpub/liuw_sql/sql_update_'+user_name+'_'+project_type+'.txt'
...
def push_file(physical_dir_path, pid, owner_user_uuid, user_name, project_type, flag='test'):
    if os.path.exists(physical_dir_path):
        ...
        feedback = subprocess.Popen('/home/cloudpub/liuw_sql/judgeFileType %s' % physical_dir_path, stdout=subprocess.PIPE, shell=True)
...
if __name__ == '__main__':
    ...
    newParser.add_argument( '-env', dest='env', help='envirenment:dev/rtm/online/yz', default='yz' );

    #hg:   mysql -h10.2.12.1 -ucloud_read -p123.bmk.liuzm  --default-character-set=utf8
    #yz:   mysql -h10.19.1.250 -ucloud -p123.bmk  --default-character-set=utf8
    ...
    #mysql --host=10.3.128.50 --user=dev_cloud --password=dev_cloud.123 dev_cloud < /home/cloudpub/liuw_sql/sql_update_ref_dev_noref.txt
    #mysql --host=10.3.128.50 --user=dev_cloud --password=dev_cloud.123 dev_biocloud_files < /home/cloudpub/liuw_sql/sql_insert_ref_dev_noref.txt

    #mysql --host=10.3.129.50 --user=rtm_cloud --password=123.rtm_cloud..bmk rtm_biocloud_files < /home/cloudpub/liuw_sql/sql_insert_xuqing1_rtm_ref.txt
    ...
    if env=='yz':
        ...
        user1 = 'cloud'
        passwd1 = '123.bmk'
        file_db1 = 'biocloud_files'
        modules = 'http://10.19.1.252:8091'
    
    temp_file = '/home/cloudpub/liuw_sql/sql_insert_'+user_name+'_'+project_type+'.txt'
    os.system('rm '+temp_file)
    temp_file1 = '/home/cloudpub/liuw_sql/sql_update_'+user_name+'_'+project_type+'.txt'
    os.system('rm '+temp_file1)
    log_file = '/home/cloudpub/liuw_sql/'+user_name+'_'+project_type+'.log'
    ...
    conn_biocloud_files = MySQLdb.connect(host=host1, user=user1, passwd=passwd1, db=file_db1, charset="utf8")
        ...
        for k in project_list.keys():
            if project_dict[p] == k:
                ...
                if push_xml_list:
                    ...
                    if file_uuid_flag == '':
                        os.system('rm -rf /share/nas2/genome/cloud_soft/python/push_%s.xml' % env)
                        generate_push_xml_cmd = 'curl -X POST --header "Content-Type: application/json" --header "Accept: application/json;charset=UTF-8" -d "{\\"createXmlPath\\": \\"/share/nas2/genome/cloud_soft/python/push_%s.xml\\",\\"jobName\\": \\"%s\\",\\"parentUuid\\": \\"%s\\",\\"userId\\": \\"%s\\"}" "%s/api/v2/files/push/createXml"' % (env,root_path.rstrip('/').split('/')[-2],biomarker_project_uuid_list[0],user_uuid,modules)
                        write_file(log_handle, generate_push_xml_cmd)
                        os.system(generate_push_xml_cmd)
                        os.system('cp /share/nas2/genome/cloud_soft/python/push_%s.xml %s' % (env,root_path+'/../'))
                ...
                if project_type == 'noref':
                    os.system('mkdir -p %s' % root_path+'/../Needed_Data/Config/')
                    if not os.path.exists(root_path+'/../Needed_Data/Config/detail.cfg'):
                ...
                if project_has_needed_data_dic.has_key(p):
                    ...
                    needed_data_uuid = push_only_one_dir(root_path+'/../Needed_Data', p, user_uuid, user_name, project_type, run_mode)
                    if needed_data_uuid != '':
                        push_dir(root_path+'/../Needed_Data', needed_data_uuid, user_uuid, user_name, project_type, run_mode)
                    else:
                        write_file(log_handle, '[[ERROR]] %s not exists!!' % (root_path+'/../Needed_Data'))
                file_not_in_db_then_push(cursor1, root_path+'/../Needed_Data/biomarker_htmlReport.zip', needed_data_uuid, user_uuid, user_name, project_type, run_mode)
                file_not_in_db_then_push(cursor1, root_path+'/../Needed_Data/biomarker_Web_Report.zip', needed_data_uuid, user_uuid, user_name, project_type, run_mode)
                ...
                if project_type == 'ref':
                    ...
                    os.system('/share/nas2/genome/biosoft/perl/current/bin/perl %s -prt %s  -app %s > /dev/null' % (new_init_script, root_path, project_type))
                    #perl /share/nas1/niulg/pipeline/tools/trans_metadata.pl -prt Web_Report/ -app ref
                    
                    nul_seq_pos1 = glob.glob(root_path+'/../Needed_Data/*.longest_transcript.fa')
                    nul_seq_pos2 = glob.glob(root_path+'/../Needed_Data/Allgene_annoNseq/*.longest_transcript.fa')
                    #推送这个文件放到了下边
                    if nul_seq_pos1 and not nul_seq_pos2:
                        os.system('cp '+nul_seq_pos1[0]+' '+root_path+'/../Needed_Data/Allgene_annoNseq/')
                    ...
                    if not os.path.exists(root_path+'/gene_pos.list'):
                        os.system('perl /share/nas2/genome/cloud_soft/app_pipeline/ref_trans_pipeline/v2.0.2/bin/extract_gene_position_gff.pl -i %s -o %s' % (root_path+'/../Needed_Data/all.gff', root_path))
                    file_not_in_db_then_push(cursor1, root_path+'/gene_pos.list', web_report_dic[p], user_uuid, user_name, project_type, run_mode)
                    file_not_in_db_then_push(cursor1, root_path+'/../Needed_Data/PPI.txt', needed_data_uuid, user_uuid, user_name, project_type, run_mode)
                    ...
                    if project_has_needed_data_dic.has_key(p) and needed_data_uuid != '':
                        ...
                        try:
                            ...
                            nul_file_phy_path = glob.glob(root_path+'/../Needed_Data/Allgene_annoNseq/*longest_transcript.fa')
                        ...
                        file_not_in_db_then_push(cursor1, root_path+'/../Needed_Data/all.gff', needed_data_uuid, user_uuid, user_name, project_type, run_mode)
                        file_not_in_db_then_push(cursor1, root_path+'/../Needed_Data/chroDetail.txt', needed_data_uuid, user_uuid, user_name, project_type, run_mode)
                ...
                if project_type == 'srna':
                    ...
                    os.system('/share/nas2/genome/biosoft/perl/current/bin/perl %s -prt %s > /dev/null' % (new_init_script, root_path))
                    ...
                    try:
                        ...
                        cursor1.scroll(0,mode='absolute')
                        file_not_in_db_then_push(cursor1, root_path+'/../Needed_Data/Config/detail.cfg', config_uuid, user_uuid, user_name, project_type, run_mode)
                    except:
                ...
                if project_type == 'noref':
                    ...
                    os.system('/share/nas2/genome/biosoft/perl/current/bin/perl %s -prt %s > /dev/null' % (new_init_script, root_path))
                    os.system('/share/nas2/genome/biosoft/perl/current/bin/perl %s -i %s/../Needed_Data/Unigene.pep.fa > /dev/null' % (modify_pep_name_script, root_path))
                    file_not_in_db_then_push(cursor1, root_path+'/../Needed_Data/Unigene.fa', needed_data_uuid, user_uuid, user_name, project_type, run_mode)
                    file_not_in_db_then_push(cursor1, root_path+'/../Needed_Data/Unigene.pep.fa', needed_data_uuid, user_uuid, user_name, project_type, run_mode)
                    file_not_in_db_then_push(cursor1, root_path+'/../Needed_Data/final.snp.list', needed_data_uuid, user_uuid, user_name, project_type, run_mode)
                    file_not_in_db_then_push(cursor1, root_path+'/../Needed_Data/PPI.txt', needed_data_uuid, user_uuid, user_name, project_type, run_mode)
                    try:
                        cursor1.execute('select file_uuid, name, owner_user_uuid, type, pid from t_file where owner_user_uuid="%s" and pid="%s" and name="Config" and recycle!=1 and field_1!="delete"' % (user_uuid, needed_data_uuid))
                        ...
                        if config_uuid[0]:
                            file_not_in_db_then_push(cursor1, root_path+'/../Needed_Data/Config/detail.cfg', config_uuid[0], user_uuid, user_name, project_type, run_mode)
                            file_not_in_db_then_push(cursor1, root_path+'/../Needed_Data/Config/data.cfg', config_uuid[0], user_uuid, user_name, project_type, run_mode)
                            cursor1.scroll(0,mode='absolute')
                        else:
                            config_uuid = push_only_one_dir(root_path+'/../Needed_Data/Config', needed_data_uuid, user_uuid, user_name, project_type, run_mode)
                            push_dir_files(root_path+'/../Needed_Data/Config', config_uuid, user_uuid, user_name, project_type, run_mode)
                ...
                if personal_analysis_dic.has_key(p) and DEG_Analysis_dic[p]:
                    ...
                    personal_analysis_uuid = personal_analysis_dic[p]
                    os.system('mkdir -p '+root_path+'/../Personal_Analysis/DEG_Analysis')
                    deg_analysis_uuid = push_only_one_dir(root_path+'/../Personal_Analysis/DEG_Analysis', personal_analysis_uuid, user_uuid, user_name, project_type, run_mode)
                else:
                    os.system('mkdir -p '+root_path+'/../Personal_Analysis/DEG_Analysis')
                    personal_analysis_uuid = push_only_one_dir(root_path+'/../Personal_Analysis', p, user_uuid, user_name, project_type, run_mode)
                    deg_analysis_uuid = push_only_one_dir(root_path+'/../Personal_Analysis/DEG_Analysis', personal_analysis_uuid, user_uuid, user_name, project_type, run_mode)
                ...
                if not ref_trans_deg and os.path.exists(root_path+'/../Personal_Analysis/DEG_Analysis/'+deg_config_file):
                    push_dir_files(root_path+'/../Personal_Analysis/DEG_Analysis', deg_analysis_uuid, user_uuid, user_name, project_type, run_mode)
                elif not ref_trans_deg and not os.path.exists(root_path+'/../Personal_Analysis/DEG_Analysis/'+deg_config_file):
                    exe_flag = os.system('/share/nas2/genome/biosoft/perl/current/bin/perl  %s --project %s' % (deg_init_script,root_path))
                    if exe_flag != 0:
