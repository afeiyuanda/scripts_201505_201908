...
page.addCSS('src/css/bootstrap.min.css',
            'src/css/index.css',
...
page.addJS('src/js/jquery-1.11.3.min.js', 
           'src/js/nav.js',
           'src/js/raxus-slider.min.js',
           'src/js/fancyBox/jquery.fancybox.pack.js',
           'src/js/fancyBox/jquery.mousewheel-3.0.6.pack.js',
...
head << img( src="src/images/logo.jpg", cl="pull-right")
...
#                     firstH1=False
                ...
                if not os.path.exists(dout+'/src/images/'+pic_name):
                    new_name=pic_name
                    convert_pic(path,dout+'/src/images/'+pic_name,workDir,args.width)
                else:
                    new_name=str(uuid.uuid4())+"."+pic_name.split('.')[-1]
                    sys.stderr.write("same pic name %s,new name will give %s \n"%(pic_name,new_name))
                    convert_pic(path,dout+'/src/images/'+new_name,workDir,args.width)
                    
                pic_p=sect <<p(cl="text-center")
                a_pic=pic_p<<a(cl='img-toggle' ,href='src/images/'+new_name,title=name)
                
                if (type in picTypes ):
                    a_pic<<img(cl=type, src='src/images/'+new_name, alt=pic_name) 
                else:
                    sys.stdout.write('pic view type of \"%s\" about %s not predefined,\"img-width-max\" will be used\n'%(type,path))
                    a_pic<<img(cl="img-width-max", src='src/images/'+new_name, alt=pic_name)
...
#                             shutil.copyfile(workDir+'/'+path,dout+'/src/images/'+pic_name)
                    ...
                    imgA=imgLi<<a(cl="img-toggle", href='src/images/'+new_name ,title=name)
                    imgA<<img(src='src/images/'+new_name)
...
divFooter<<p(u'Copyright © 2009-2015 北京百迈客生物科技有限公司版权所有  京ICP备10042835号'.encode("UTF-8"))
...
ulfooter<<li(u"微信:biomarker_tech".encode("UTF-8"))
ulfooter<<li("<a href=\"http://www.biocloud.cn/external/login/toLogin\" target=\"_blank\">百迈客生物云平台</a>")
ulfooter<<li("<a href=\"http://www.biomarker.com.cn/guanywm.php\" target=\"_blank\">关于我们</a>")

#######################################################################
divgoTop=container_shadow<< div(id="goTop")
divgoTopA=divgoTop<<a(cl="backtotop" ,title="Top")
divgoTopA<<img(src="src/images/goTop.jpg", cl="back-tip")
